...
[[ -e build/cluster-api-provider-metal3 ]] || git clone https://github.com/metal3-io/cluster-api-provider-metal3.git build/cluster-api-provider-metal3/
git -C build/cluster-api-provider-metal3 checkout master
git -C build/cluster-api-provider-metal3 pull --tags

releases() {
    git -C build/cluster-api-provider-metal3 tag | sort -rV | while read TAG; do
        echo "$TAG"
    done
}
...
features_for() {
    echo -e ""
    git -C build/cluster-api-provider-metal3 show $1 | grep Date: | head -n1 | sed "s/Date:\s\+/Released on: /"
    echo -e ""
    git -C build/cluster-api-provider-metal3 show $1 | sed -n "/changes$/,/Contributors/ p" | egrep "^- "
}
